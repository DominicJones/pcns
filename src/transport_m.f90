  ...
  do i = 1, dmn%n_fce
    ...
    case (1:)
      ...
      if (jn >= dmnl .and. jn < dmnu) then
        ...
        ! pde%mat%val(kii) = pde%mat%val(kii) + max(small, - c_ij - d_ij)
        ! pde%mat%val(kjj) = pde%mat%val(kjj) + max(small, - c_ji - d_ij)

        pde%mat%val(kii) = pde%mat%val(kii) + max(small, - c_ij)
        pde%mat%val(kjj) = pde%mat%val(kjj) + max(small, - c_ji)
        pde%mat%val(kii) = pde%mat%val(kii) + max(small, - d_ij)
        pde%mat%val(kjj) = pde%mat%val(kjj) + max(small, - d_ij)
